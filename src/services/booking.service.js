...
const BookingService = {
    ...
    getBookingToId: async (id) => {
        try {
            const response = await $api.get(`/booking/corp/status/${id}`);
    ...
    createBookingCorp: async (path, numberId,dataBooking, dataNumber,userId, updateUser) => {
        try {
            const response = await $api.post(`/${path}/booking/corp`, {numberId, dataBooking, dataNumber,userId, updateUser});
            return response.data;
        } catch (error) {
            throw new Error('Failed to create booking from booking table');
        }
